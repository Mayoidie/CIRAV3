        ...
        {activeTab === 'my-tickets' && (
          <motion.div key="my-tickets" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
            <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
                ...
                <button onClick={() => setMyTicketsFilter('rejected')} className={`px-4 py-2 rounded-lg transition-all whitespace-nowrap ${myTicketsFilter === 'rejected' ? 'bg-[#FF4D4F] text-white' : 'bg-white text-[#7A7A7A] border border-gray-300 hover:bg-gray-50'}`}>Rejected ({rejectedMyTickets.length})</button>
            </div>
